...
def get_article_by_id(request, id):
    ...
    # return HttpResponse(f"Article text with article id {id} <br> {article.article_text}")

    article = get_object_or_404(Article, pk=id)
    return render(request, 'newsapp/article.html', {"article":article})

def search_article_by_text(request, text):
    articles = list(Article.objects.filter(article_text__contains=text).values_list())
    print(articles)
    try:
        result=""
        for art in articles:
